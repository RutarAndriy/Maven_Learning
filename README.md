...
При детальному вивченні pom-файлу можна помітити, що для опису деяких залежностей (таких як бібліотеки `Junit` та `TestNG`) використовується параметр `<scope>`, він же - область видимості. Даний параметр дозволяє вказати Maven'у коли і для чого нам потрібна дана залежність. Всього існує шість областей видимості:
