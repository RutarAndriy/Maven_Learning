...
Як бачимо, виник конфлікт залежностей, оскільки у проекті використовується залежність образу двох версій: `1.3` та `2.2`. У цьому випадку Maven залишить у проекті ту залежність, яка має виший порядок, тобто `hamcrest-core-2.2`. Інша версія бібліотеки буде виключена із дерева залежностей.
